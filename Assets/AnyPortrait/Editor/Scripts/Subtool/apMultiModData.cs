...
namespace AnyPortrait
{
	...
	public class apMultiModData
	{
		...
		public class LinkUnit
		{
			...
			public void SetModMesh(apModifiedMesh modMesh, apModifierBase modifier)
			{
				...
				if (_linkedRenderUnit != null
					&& _linkedRenderUnit._renderVerts != null
					&& _linkedRenderUnit._renderVerts.Count > 0)
				{
						...
						&& _modMesh._vertices.Count > 0)
					{
						...
						isSameSize = _modMesh._vertices.Count == _linkedRenderUnit._renderVerts.Count;
						
						for (int iRV = 0; iRV < _linkedRenderUnit._renderVerts.Count; iRV++)
						{
							...
							&& _modMesh._vertRigs.Count > 0)
					{
						...
						isSameSize = _modMesh._vertRigs.Count == _linkedRenderUnit._renderVerts.Count;
						...
						&& _modMesh._vertWeights.Count > 0)
					{
						...
						isSameSize = _modMesh._vertWeights.Count == _linkedRenderUnit._renderVerts.Count;
		...
		private const int MRV_INIT_POOL_SIZE = 200;
		private const int MRV_ADD_POOL_SIZE = 100;
		...
		{
			if(_mrvPool_All == null)
			{
				_mrvPool_All = new List<apSelection.ModRenderVert>();
			}
			if(_mrvPool_Live == null)
			{
				_mrvPool_Live = new List<apSelection.ModRenderVert>();
			}
			if(_mrvPool_Ready == null)
			{
				_mrvPool_Ready = new List<apSelection.ModRenderVert>();
			}
			_mrvPool_All.Clear();
			_mrvPool_Live.Clear();
			...
			//Pool 크기를 처음으로 설정하자
			AddMRVUnits(MRV_INIT_POOL_SIZE);
		...
		public apSelection.ModRenderVert GetMRV(apTransform_Mesh meshTransform, apVertex vert)
		{
			...
			{
				if(_vert2MRV[meshTransform].ContainsKey(vert))
				{
					return _vert2MRV[meshTransform][vert];
				}
			}
			return null;
		}
