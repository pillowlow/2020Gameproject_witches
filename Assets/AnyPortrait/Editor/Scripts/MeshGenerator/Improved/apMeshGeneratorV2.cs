...
namespace AnyPortrait
{
	...
	public partial class apMeshGeneratorV2
	{
		...
		public void AddRequest(apMesh mesh)
		{
			...
			GenRequest newRequest = new GenRequest(iNext, mesh, newResult);
		...
		private void Processing_Step6_MakeOuterEdges_Wrap(bool isFirstFrame)
		{
			...
			for (int iWrap = 0; iWrap < _wrapGroups.Count; iWrap++)
			{
				...
				float maxEdgeLengthForDivide = Mathf.Min(90.0f * curWrapGroup._areaSizeRatio, curWrapGroup._areaShortSize / (float)_option_Inner_Density);
		...
		{	
			...
			bool isInnerMargin = _option_IsInnerMargin;//내부 여백이 있는가
			...
			for (int iGroup = 0; iGroup < _wrapGroups.Count; iGroup++)
			{
				...
				float innerVertRange = curGroup._areaShortSize / (float)_option_Inner_Density;//이거에 따라 밀집도가 다름.
				...
				try
				{
					for (int iData = 0; iData < nOut2InLineData; iData++)
					{
						...
						float sqrMinNearRange = (_option_InnerMargin + _option_OuterMargin);
		...
		{	
			...
			bool isInnerMargin = _option_IsInnerMargin;
