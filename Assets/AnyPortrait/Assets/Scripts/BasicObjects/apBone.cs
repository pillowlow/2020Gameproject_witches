...
namespace AnyPortrait
{
	...
	public class apBone
	{
		...
		public void MakeWorldMatrix(bool isRecursive/*, bool isDebug = false*/)
		{
			...
			#region [미사용 코드] apMatrix와 apComplex를 통해서 직접 계산하는 방식
			////-----------------------------------
			//// 기존 RMultiply 방식
			////-----------------------------------
			////if (_parentBone == null)
			...
			////	//추가 20.8.5 : RMultiply 전에 함수를 호출해야한다. [RMultiply Scale 이슈]
			...
			////-----------------------------------
			//// 변경 20.8.12 : RMultiply + Skew Multiply 모두 지원 [Skew 이슈]
			//// apMatrix > apComplexMatrix로 바뀌었다.
			//// 대부분의 함수는 유사하다.
			////-----------------------------------

			//if (_parentBone == null)
			//{
			//	_worldMatrix.SetMatrix_Step1(_defaultMatrix, false);
			//	_worldMatrix_NonModified.SetMatrix_Step1(_defaultMatrix, false);
			//	if (_isRigTestPosing)
			//	{
			//		_worldMatrix.Add(_rigTestMatrix);//Rig Test 추가
			//	}
			...
			//	//추가 20.8.5 : RMultiply 전에 함수를 호출해야한다. [RMultiply Scale 이슈]
			//	_worldMatrix.OnBeforeMultiply();
			//	_worldMatrix_NonModified.OnBeforeMultiply();
			...
			//		//<중요> 여기서 RMultiply 대신 SMultiply를 넣어주는게 핵심
			//		//true를 넣어저 MakeMatrix도 완성하자
			//		_worldMatrix.SMultiply(_renderUnit.WorldMatrixWrap, true);
			//		_worldMatrix_NonModified.SMultiply(_renderUnit.WorldMatrixWrapWithoutModified, true);
			//	}
			//	else
			//	{
			...
			//	//<중요> 부모 본이 SMultiply까지 적용된 행렬일 수 있다.
			//	//따라서 RMultiply와 SMultiply가 모두 포함된 ComplexMultiply로 연산하자
			//	//true를 넣어서 MakeMatrix도 실행하자
			//	_worldMatrix.ComplexMultiply(_parentBone._worldMatrix, true);
			//	_worldMatrix_NonModified.ComplexMultiply(_parentBone._worldMatrix_NonModified, true);
			//} 
			#endregion
